...
- **ガチャUI 実装（2026-06-14・S001限定公開）**: `src/app/(portal)/gacha/page.tsx`（`GACHA_ALLOWED=["S001"]` 以外は `/dashboard` redirect）＋`GachaClient.tsx`（RPG風・単発100/10連1000・がしゃ演出＋★レア枠＋NEW・所持グリッド・タップで連れ歩き設定）。ナビ「ガチャ」は `layout.tsx` の `staffMenu`(staffId==="S001"のみ追加)。アイコンは `Trophy`。**全員開放時**: page.tsx の `GACHA_ALLOWED` 撤去＋layout の S001 分岐撤去。`drawGachaAction`/`getGachaStateAction`/`setActivePartnerAction` 接続済
