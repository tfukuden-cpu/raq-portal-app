...
## 現在の開発状態（2026-09-26更新）

### 休憩シートの #REF! 検知＋座席と休憩の食い違いアラート（2026-09-26）
**①ユーザー報告「⚠️ シートに 2026-09-26 の行がありません」→ 調査の結果アプリのバグではなく、休憩表シートの `IMPORTRANGE` が一時的に切れて中身が `#REF!` 1セルだけになっていた（その後シートを開き直したら自然復旧・44行/全て9-26・見出し3行・未解決0）。②続けてユーザー依頼「座席にいるのに休憩シートに名前がない人の座席を赤くしてほしい／逆に休憩シートにいて座席にいない人もずっと分かるように。ただし欠勤なら無くて正解」。実装は engineer、検品は qa-reviewer。tsc 0・`npx eslint src` 0 errors・warning 76（増減なし）・`next build` 成功。**
- **`#REF!` 検知（デプロイ済 `dcd5bce`）**: `break-actions.ts` に `findSheetError()` を追加。**休憩の時刻（D/E列）が入った行が1行も無く、かつエラー値（`#REF!`/`#N/A`/`#ERROR!` 等）があるとき**だけ「シートが #REF! になっています（IMPORTRANGE の連携が切れています）。シートを開いて『アクセスを許可』してから…」を返す。正常なシートに注記でエラー値が混ざっても取り込みは止まらない
- **座席と休憩の食い違いアラート**: `SeatingClient.tsx`（＋`seating/page.tsx`・`attendance/page.tsx` に各2行）。**共通ガード `hasBreakData = Object.keys(breakAssignmentMap).length > 0`** を①②両方のAND条件に入れ、**休憩割当0件の日は完全に沈黙**する（翌日プランは別コンポーネント `plan/SeatingPlanClient.tsx` なのでそもそも通らない）
  - **①席あり・休憩なし＝赤リング＋最下段の赤帯「休憩なし」**。`ring-2 ring-red-500` を `cardBorder` の後ろに重ねるだけで**ステータス色（`CARD_BODY_BG`）もセクション色枠も書き換えない**。除外＝`editMode`／`isNotWorking`（オレンジ破線を優先）／`status==="absent"`／**査定・販売以外**（`BREAK_TARGET_SECTIONS`）
  - **②休憩あり・席なし＝未配置パネルで赤強調＋「休憩①」**。欠勤は赤くせずグレーで「欠勤」
- **ユーザー判断（2026-09-26）**: **MOTAは赤くしない**＝休憩スロットの運用対象は査定/販売のみ（SVのシートが査定/販売しか載せないため、MOTA 2席が毎日赤くなり本当に見たい数名が埋もれる）。`BREAK_TARGET_SECTIONS = ["査定","販売"]` をモジュール定数に切り出し、**休憩一覧パネルのローカル `TARGET_SECTIONS` も削除してこれに統一**（判定が画面内でばらつかない）
- **本番データでの発火量（9/22〜9/26）**: 「席あり・休憩なし」は査定/販売で**毎日4〜11名**（MOTA除外前は毎日+2席）。「休憩あり・席なし」は**5日間で0件**＝席を外したときだけ光る例外検知
- ⚠️ **座席カードの余白はほぼ無い**: `h-[70px]`＋`border-2` は `box-sizing: border-box` で内側66px。ヘッダー18＋赤帯12＝30、ボディに残る36pxに対し実contentは35.75px＝**余裕0.25px**。ボディに1行足すと `overflow-hidden` で**最後の子である赤帯が先に切れる**
- 残: デプロイ（ユーザー承認待ち）
...
- **取り込み元シート**（`project_settings.break_sheet_url` に登録済み）: A=日付(`YYYY/MM/DD`)／B=アカウント番号(`ASS 61`)／C=商材／D=休憩開始／E=休憩終了／F=小休憩開始／G=小休憩終了。**データ行の判定は「D/E列に時刻があること」**（2026-09-26変更。見出し行が混ざり位置も動くため行番号で固定しない）。⚠️**シートは当日分が自動で出る作りで前日分は消える＝取り込みは当日中に**。9/24の実データは36名で、**休憩の時間帯はP001の既存スロット①②③と完全一致**（違うのは誰がどのスロットかだけ＝自動振り分けが実態と合っていなかった）
...
| `SeatingClient` の `statuses` は**席に座っている人しか持たない**（未配置スタッフの状態は引けない） | `statuses` の初期値は `seats.forEach(...)` ＝座席に載っている人だけで、実行中に更新する経路（Realtime の punch_logs ハンドラと `PunchModal` の `onStatusChange`）も `working/clocked_out/on_break/seat_leave` しか入れない。そのため**未配置パネルの行で `statuses` を引くと必ず `undefined`**になり、`sfStatus === "absent"` のような判定は**常に false＝到達不能なコード**になる（2026-09-26の「休憩あり・席なし」実装で qa-reviewer が検出）。欠勤など座席に依存しない属性は **`StaffInfo` に持たせて props で渡す**こと（`seating/page.tsx` の `absenceIds`／`attendance/page.tsx` の `seatAbsenceIds` から `isAbsent` を注入済み＝座席カードの `"absent"` と同じ集合なので見解がズレない） |
| 休憩表スプレッドシートは `IMPORTRANGE` で元シートを引いている＝`#REF!` で空になることがある | 元の休憩表を `IMPORTRANGE("…","休憩時間!A1:G")` で1セルから展開しているため、**元シート側の共有（アクセス許可）が切れると中身が `#REF!` 1セルだけになる**（2026-09-26に発生。A1のツールチップが「このスプレッドシートにアクセスする権限がありません」）。この状態で取り込むと行が0件なので「シートに YYYY-MM-DD の行がありません」と出て、アプリのバグに見える。`findSheetError()`（break-actions.ts）で**休憩の時刻が入った行が1行も無く、かつエラー値がある**ときだけ検知し「IMPORTRANGE の連携が切れています／シートを開いて『アクセスを許可』」と出す。**復旧はシート側の作業でアプリからは直せない**（Google連携アカウントに共有されていても、IMPORTRANGE の許可は別） |
| 休憩表スプレッドシートは「当日分だけ」が出る／行番号でデータ開始位置を決めない | シート先頭に IMPORT の注記があり、**表示されるのは当日分のみ＝前日分は翌日には消える**（2026-09-26に確認。9/25の37行が翌日には無く9/26の44行に入れ替わっていた）。**取り込みは当日中に行うこと**。また見出し・注記の行（「査定番付」「アカウント」「11」など）が混ざり、日付ブロックが変わると位置も動くため、**データ行の判定は「D/E列に休憩の時刻が入っているか」で行う**（固定の開始行にすると見出しを未解決として毎回出すか、データを取りこぼす）。時刻はあるのにアカウント番号や日付が読めない行だけ未解決に出す |
