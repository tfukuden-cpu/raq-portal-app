...
- **追補3＝日毎の欠勤者を専用ページ化（デプロイ済 ad417d7→04b13ae）**: ユーザー要望「日毎の欠勤者を別ページに・表は名前羅列でデザイン性に欠ける→日毎/月毎を選ぶ形に」。欠勤者レポートのインライン日毎リストを廃止→レポートタブにカード型ボタン（2段ラベル「日毎の欠勤者／カレンダー・月毎一覧で確認」・「表で見る」文言は不可）で新ページ `/attendance/absentees`（`page.tsx`管理者ガード＋`AbsenteeDailyClient.tsx`・`max-w-2xl`）へ。**上部に日毎/月毎セグメント切替**: 日毎=月カレンダー（各日に欠勤人数バッジ・日タップで下に欠勤者一覧・最終欠勤日を初期選択）／月毎=その月の欠勤日を日付カードで一覧。同じ `/api/admin/work-records/absentees`(byDate)を再利用。戻るリンク`/attendance/edit?tab=absentees`
- 残: 全件デプロイ済
