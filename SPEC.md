...
> **ガチャ画面 `/gacha`（2026-06-14）**: `page.tsx`（`GACHA_ALLOWED=["O002"]`＝福傳のみ・以外は `/dashboard` へ redirect）＋ `GachaClient.tsx`（RPG風・ガチャマシン画像 `public/rpg/gacha-machine.png`＋がしゃ演出・単発100/10連1000・★レアリティ枠＋NEWバッジ・所持パートナーグリッド・タップで連れ歩き設定）。ナビ項目「ガチャ」も `layout.tsx` で `staffId==="O002"` のみ表示（`staffMenu`）。**全員開放時は page.tsx の `GACHA_ALLOWED` 撤去＋layout の O002 分岐を撤去**。※福傳の実アカウントは **O002**（executive）。S001は別のテスト用。テスト用に O002 のコインを 99,999,999 に設定済
