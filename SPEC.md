...
- 「きゅうけいしつ」ウィンドウ: 空き数（あき N／M）と箱グリッドを常時表示。空き箱タップ→確認→本人が入室（`enterMyBreakRoomAction`・休憩打刻中のみサーバー側で検証）。自分の箱（アンバー枠「じぶん」）タップ→退室。他人の箱は名前＋キャラ＋入室時刻表示のみ。**管理者は「▶閉鎖する／開放する」ボタンで開閉切替**（`break_room_settings.is_open`・閉鎖中は全箱✕表示＆入室不可・打刻端末側も「とざされている」表示で入室ブロック）
