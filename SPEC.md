...
- **座席と休憩の食い違いアラート（2026-09-26追加）**: シート取り込み運用に変わり「座席にいるのに休憩が無い人」「休憩はあるのに座席にいない人」が出るようになったため、両方向を画面に出す。**どちらも休憩の割り当てが1件も無い日は一切出さない**（`hasBreakData` ＝ 取り込み前に全座席が赤くなるのを防ぐ）
  - **①席あり・休憩なし** → 座席カードに**赤いリング＋最下段の赤帯「休憩なし」**。カード中身のステータス色（勤務中=緑／休憩中=橙／離席=グレー／超過=赤）とセクション色の枠は変えない。**対象は査定・販売のみ**（`BREAK_TARGET_SECTIONS`・SVのシートが査定/販売しか載せないため、他セクションは毎日赤になりノイズになる。休憩一覧パネルと同じ定数を共有）。**席替えモード中・未出勤（オレンジ破線を優先）・欠勤は出さない**
  - **②休憩あり・席なし** → 右の「未配置」パネルの該当行を**赤く強調しスロット記号（休憩①）を併記**。席替えモード中も出す。**欠勤の人は赤くせずグレーで「欠勤」**（欠勤なら座席が無いのは正常）
  - ⚠️ **未配置パネルの欠勤判定に `statuses` を使ってはいけない**（`StaffInfo.isAbsent` を props で渡す）。理由は `memory/MEMORY.md` の地雷表を参照
...
#### シフト仮組（自動生成）
シフトの下書き（`shift_grid_drafts.draft_data`）を自動生成する。実装は `admin/[projectId]/settings/draft-actions.ts` の `generateShiftDraftAction`。設定ページの「シフトの仮組」とシフト管理グリッドの「再仮組み（全体／セクション別）」の両方が同関数を使う。

- **必要人数**: `shift_slot_requirements`（パターン×日付）。未設定はパターンの平日/休日必要数→スタッフ数からの自動算出にフォールバック
- **希望休**: `shift_off_requests`（優先度付き・実運用テーブル）を参照し、申請日はそのスタッフを当日除外する。※`holiday_requests`（承認制・別系統）ではない
- **手動編集セクションの除外**: `インフォ`（パターン「販売/インフォ」）と `H MOTA`（ヘルプMOTA）は自動割当しない（`lib/shift-draft-config.ts` の `MANUAL_DRAFT_SECTIONS`）。`販売`・査定・MOTA等は自動対象。除外セクションは設定ページの必要人数グリッド・再仮組みモーダルの選択肢からも非表示
- **割当制約**: セクション/ロール一致・同日重複なし・月/週稼働日数上限・勤務間インターバル11時間以上・遅番翌日に早番を入れない・連勤上限（既定5・`project_members.max_consecutive_days`）。連勤は当日の**前後**の連続区間を合算して判定する（飛び飛び割当で区間が連結し上限超過するのを防ぐ）
- **対象外**: `work_days_type='spot'`、研修日（`staff_trainings`・研修日のみ研修系パターン可）
- **仮保存／続きから編集**: グリッドの「仮保存」は `draft_data` を upsert。仮保存成功時に親へ通知し下書きを最新化するため、閉じて「続きから編集」しても保存内容が反映される
...
- シートの形: A=日付（`YYYY/MM/DD`）／B=アカウント番号（`ASS 61`・ゼロ埋めあり）／C=商材／D=休憩開始／E=休憩終了／F=小休憩開始／G=小休憩終了
- **データ行の判定は「休憩の時刻（D/E列）が入っていること」**（2026-09-26変更）。シートには見出し・注記の行（「査定番付」「アカウント」「11」など）が混ざり、**日付ブロックが変わると位置も動く**ため、行番号で固定しない。時刻が無い行は見出しとして黙ってスキップし、**時刻はあるのにアカウント番号や日付が読めない行だけ未解決として画面に出す**
- ⚠️ **シートは「当日分が自動で出る」作り**（先頭に IMPORT の注記あり）。**前日分は翌日には消える**＝取り込みはその日のうちに行うこと。過去日を取り込みたい場合はシートの内容を別途控えておく必要がある
- ⚠️ **このシートは元の休憩表を `IMPORTRANGE` で引いている**＝元シート側の共有が切れると**中身が `#REF!` 1セルだけになる**（2026-09-26に発生）。この状態を検知して「シートが #REF! になっています（IMPORTRANGE の連携が切れています）。シートを開いて『アクセスを許可』してください」と出す。**復旧はシート側の作業**（アプリからは直せない）
...
- **連れ歩き**: 所持パートナーから1体を「現在のパートナー」に設定（`staffs.active_partner_id`）。ホーム表示用（パーティーとは別管理）

#### B-2. バトル/育成（土台・`src/lib/monster-stats.ts`）

- **6ステータス**: HP / こうげき / ぼうぎょ / とくこう / とくぼう / すばやさ
- **8属性(+無)**: 火 水 草 雷 氷 地 風 闇（相性でダメージ増減＝`elementMultiplier`・暫定）
- **5ロール**: アタッカー/タンク/スピード/まほう/バランス（基礎値の配分が変わる）
- **基礎能力値**: レア度の合計（★1:180〜★5:470）× ロール配分 × id個体差。`baseStats(id)`／図鑑に表示
- **レベル＆努力値**: Lv1〜50・経験値で成長。`能力値=基礎値×レベル補正＋努力値`。レベルアップごとに努力値+3pt（1ステ最大100・合計最大150）。育成は**所持インスタンス単位**（`staff_partners`の行）
- 各150体に `属性・ロール・図鑑説明` を設定（`monster-stats.ts` の BATTLE_DATA・画像準拠）
