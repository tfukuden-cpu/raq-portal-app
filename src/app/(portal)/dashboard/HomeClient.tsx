...
function StatusBadge({ state }: { state: HomeState }) {
  ...
  const c = map[state];
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-semibold ${c.cls}`}>
      {state === "working" && (
  ...
  return (
    <>
      <main className="min-h-screen bg-[#F5F7FA] dark:bg-zinc-950">
        <div className="max-w-6xl mx-auto px-4 md:px-8 pt-6 md:pt-8 pb-32 md:pb-12">

          {/* ── ヘッダー：日付 + クロック + ステータス ── */}
          <div className="flex items-end justify-between mb-6 md:mb-8">
            <div>
              <p className="text-[13px] text-zinc-400 mb-0.5">{todayLabel}</p>
              <p className="text-[48px] md:text-[56px] font-bold tabular-nums leading-none tracking-tight text-zinc-900 dark:text-white">
                {liveTime}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                <div className="px-5 pt-5 pb-3 flex items-center justify-between">
                  <h2 className="text-[15px] font-semibold text-zinc-800 dark:text-zinc-100">お知らせ</h2>
