...
import { unlinkLineAction } from "@/app/(portal)/my/actions";
...
export default async function AdminMyPage({
  ...
  if (!staff || (staff.global_role !== "executive" && staff.global_role !== "admin")) {
    redirect("/dashboard");
  }

  const displayName  = staff.display_name ?? staff.name ?? staffId;
  const lineLinked   = !!staff.line_user_id;
  const avatarConfig = (staff as { avatar_config?: AvatarConfig | null }).avatar_config ?? null;

  const roleLabel = staff.global_role === "executive" ? "運用者" : "管理者";
  const roleBadgeClass = staff.global_role === "executive"
  ...
  return (
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <div className="max-w-lg mx-auto px-4 pt-6 pb-28 space-y-5">
        ...
        {/* ── アカウント設定 ── */}
        <div>
          <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider px-1 mb-2">
            アカウント
          </p>
