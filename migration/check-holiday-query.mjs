...
// generateShiftTableAction と同じクエリ
...
console.log(`クエリ範囲: ${dateFrom} 〜 ${dateTo}`);

const { data, error, count } = await supa
  .from("holiday_requests")
  .select("staff_id, request_date, note", { count: "exact" })
  .eq("project_id", "P001")
  .gte("request_date", dateFrom)
  .lte("request_date", dateTo)
  .order("request_date");

console.log("取得件数:", count);
if (error) console.error("エラー:", error.message);
else console.table(data?.slice(0, 5));
