...
for (let i = 0; i < TEST_STAFFS.length; i++) {
  const d = new Date(today);
  d.setDate(d.getDate() + i + 1); // 明日〜5日後
  const dateStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
  rows.push({ project_id: PROJECT_ID, staff_id: TEST_STAFFS[i], request_date: dateStr, status: "approved", note: "テスト申請" });
}
